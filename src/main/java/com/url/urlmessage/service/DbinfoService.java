...
public class DbinfoService {
    ...
    public int addMumbers(Dbinfo dbinfo) {
        int res = jdbcTemplate.update("insert into dbinfo(name) values(?)", new PreparedStatementSetter() {
            @Override
    ...
    public List<Object> getList() {
        String sql = "SELECT '{\"id\":\"1\",\"name\":\"'+isnull(name,'-')+'\"}' as a FROM DBINFO";
        sql="SELECT '{\"url\": \"'+页面信息+'\",\"attr\": {\"urlleibie\": \"'+品类+'\",\"urlweb\": \"'+电商+'\",\"brand\": \"'+品牌+'\",\"model\": \"'+机型+'\"}}' FROM URLDATA WHERE NEED=0 AND 电商!='天猫商城'";
